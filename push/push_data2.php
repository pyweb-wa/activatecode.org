...
function Parser()
{
    ...
    try {
        ...
        if (isJson($content) == 1) {
            ...
            $req_dump = json_decode($content, JSON_UNESCAPED_UNICODE, JSON_UNESCAPED_SLASHES);
            ...
            if (isset($req_dump["results"])) {
                ...
                if (!empty($req_dump['numbers'])) {
                    ...
                    //file_put_contents($GLOBALS['logPath'] . '/_data2_.log', file_get_contents('php://input'), FILE_APPEND);
                    // echo count($req_dump['numbers']);die();
        ...
        if ($type == "sms") {
            ...
            if (isset($results["numbers"])) {
                if (!empty($results['numbers'])) {
                    ...
                    $maxAttempts = 3; // You can adjust this based on your needs
                    $attempts = 0;
                    while ($attempts < $maxAttempts) {
                        ...
                        ON DUPLICATE KEY UPDATE source = VALUES(source);";
                            ...
                            if ($e->getCode() == 40001) {
                                ...
                                $pdo->rollBack(); // Rollback to release any locks
                                $attempts++;
                                usleep(500000); // Wait for a short period before retrying (0.5 seconds in this example)
                            } else {
                                // If it's not a deadlock, rethrow the exception
                                throw $e;
                            }

                            file_put_contents($GLOBALS['logPath'] . "push_errors.log", $timestamp . ' Caught exception: ' . $e->getMessage() . " try  $attempts \n", FILE_APPEND);
                        }
                    }
                    //////////////////////////////////
