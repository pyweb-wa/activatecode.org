...
function Parser()
{
    ...
    file_put_contents($GLOBALS['logPath'].'/push/_data_' . $time_string . '.log', file_get_contents('php://input'));
        ...
        if ($type == "sms") {
            ...
            if (isset($results["numbers"])) {
                if (!empty($results['numbers'])) {
                    ...
                    $ser = $country_n."_".$results["numbers"][0]['source'];
                   $sql_del = 'DELETE FROM `bananaapi-number` WHERE `is_finished` = 0 AND `source` = "' . $ser . '";';
                    //$sql_del = 'DELETE FROM `bananaapi-number` WHERE `is_finished` = 0 AND  `country_code` = "' . $$ser . '";';
                    ...
                    try {
                        ...
                        foreach ($results["numbers"] as $number) {
                            ...
                            // $key = $country . '_' . $source;
                            // if (!isset($uniqueObjects[$key])) {
                            //     $uniqueObjects[$key] = array(
                            //         'country' => $country,
                            //         'source' => $source
                            //     );
