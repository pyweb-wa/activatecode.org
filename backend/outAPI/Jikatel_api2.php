...
class jikatel_backend
{
    ...
    {
        $sql2 = " DELETE FROM country_stats1 where country_char =? and source =?  ";
    ...
    public function GetDataStats($source = null)
    {
        ...
        $sql = "select * from country_stats1";
        if ($source != null) {
    ...
    public function download($data, $source)
    {
        try {
            ...
            $stmt = $this->pdo->prepare("SELECT phone_number from `bananaapi-number` where `is_finished` = 0 and `bananaapi-number`.`country_code` = ? and `bananaapi-number`.`source` = ? ");
            $stmt->execute([$data, $source]);
        ...
        try {
            ...
            ";
                ...
                if ($stmt->execute()) {
                    ...
                    JOIN country_stats1 AS cs ON  cc.source = cs.source
...
if (isset($obj["action"])) {
    ...
    switch ($obj["action"]) {
        ...
        case "download":
            ...
            $result = $api->download($country_char, $source);
            echo $result;
            break;
        case "powertoggle":
