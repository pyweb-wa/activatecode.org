...
class jikatel_backend
{
    ...
    public function __construct()
    {
        ...
        include_once "/var/www/smsmarket/html/backend/mylogger.php";
        $this->logger = new MyLogger();
        $this->pdo = $pdo;
        $this->path = "/var/www/smsmarket/logging/";
    }
    ...
    public function deletenumbers($obj)
    {
        try {
            ...
            $redisKey = "live_". $obj['data']['source'];
            $redis->del($redisKey);
    ...
    public function GetDataStats($source = null)
    {
        ...
        $sql = "select country_stats.*,`countries_control`.application from country_stats,countries_control where country_stats.source = countries_control.source ";
        if ($source != null) {
    ...
    public function download($obj)
    {
        try {
            $source = $obj['data']['source'];
            $createdTime = $obj['data']['pushtime'];
            ...
            $stmt = $this->pdo->prepare("SELECT phone_number from `redis_numbers` where createdTime = ?  and `redis_numbers`.`source` = ? ");
            $stmt->execute([$createdTime, $source]);
    ...
    public function powertoggle($data)
    {
        try {
            ...
            if (isset($data['data']['status'], $data['data']['country_char'], $data['data']['source'])) {
                ...
                if($status==1){
                    $redis->sAdd("countries_box:$country_char",  $source);
                }
                else {
                    $redis->sRem("countries_box:$country_char",  $source);
                }
            ...
            ";
                ...
                if ($stmt->execute()) {
                    ...
                    FROM countries_control AS cc
                    JOIN country_stats AS cs ON  cc.source = cs.source
                    WHERE cs.country_char = :country_char AND cc.enabled > 0;";
                    ...
                    if ($stmt->execute()){
                        ...
                        $redis->set("$tableName:$country_char", $status); 
