...
class banana_api
{
    ...
    public function check_enabled($country_char,$api_key,$businessCode, $source = null)
    {
        try {
            ...
            $query = "SELECT `enabled`,`source` FROM `countries_control`,`countryList` WHERE `countryList`.`id` = `countries_control`.`country_id` and `countryList`.`country_char` = ? 
            -- AND (TIME(NOW()) >= start AND TIME(NOW()) <= stop OR start = stop) 
            AND (TIME(NOW()) >= IF(start = '00:00:00', '24:00:00', start) AND TIME(NOW()) <=  IF(stop = '00:00:00', '24:00:00', stop) or start = stop)
            and enabled = 1 and `countries_control`.application = ? and source IN ($placeholders)";
              //SELECT * from `countries_control` where  TIME(NOW()) >= "22:09:00" AND TIME(NOW()) <= (SELECT IF(TIME_FORMAT("00:00:00", '%H:%i:%s') = '00:00:00', '24:00:00', TIME_FORMAT("00:00:00", '%H:%i:%s')));
    ...
    public function getnumber($businessCode, $country, $available = 0, $count = 1, $api_key = 0)
    {
        ...
        //    die();
            if ($rows) {
                ...
                $randomSleepTime = mt_rand(500000, 1500000);
