...
class banana_api
{
    ...
    {   
        ...
        try {
           ...
           if($count ==1){
            ...
            //$microtime = microtime(true); // true parameter returns float value
            //$milliseconds = round($microtime * 10000);
            // Set the base time zone (you can adjust this to your preferred base time zone)
            $baseTimeZone = 'UTC';
            ...
            $sql = "UPDATE `bananaapi-number` SET taked=1, taked_time=NOW() ,flag = ?
            WHERE `taked`=0 AND `country_code` = ? AND `application` = ? AND type = 0 AND `source` = ? limit  $count";
             //$this->pdo->beginTransaction();
             $stmt = $this->pdo->prepare($sql);
             $stmt->execute([$milliseconds ,$country, $businessCode, $randomSource]);
             // Close the cursor after the UPDATE query
             $stmt->closeCursor();
             $stmt = $this->pdo->query("Select id, phone_number FROM `bananaapi-number` WHERE flag = $milliseconds");
             $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        }
            if ($rows) {
    ...
    // GROUP BY `country_code`,`application`,`app_code`');
        // $sql = "SELECT COUNT(n.phone_number) AS count, n.application, n.country_code, f.code AS app_code FROM `bananaapi-number` AS n INNER JOIN foreignapiservice AS f ON lower(n.application) = lower(f.Name) AND n.country_code = f.country WHERE n.taked = 0 AND f.Id_Foreign_Api = 17 AND n.type = 0 GROUP BY n.country_code, n.application, f.code;";
            ...
            countryList.country,
            MAX(`countries_control`.`enabled`) as status,
            ...
            GROUP BY
                `country_stats`.`country_char`, countryList.country;';
