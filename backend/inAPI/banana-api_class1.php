...
class banana_api
{
    ...
    public function getnumber($businessCode, $country,$available=0,$hash=0,$api_key=0)
    {   
        ...
        try {
           ...
           // echo $randomSource;
            $sql = "SELECT id, phone_number FROM `bananaapi-number` WHERE `taked`=0 AND `country_code` = ? AND `application` = ? AND type = 0 AND `source` = ? LIMIT 1";
           // $sql = "SELECT id, phone_number FROM `Jikatlnumbers` where `taked`=0 and `country_code` = ? and `application` = ? ORDER BY IFNULL(`release_time`, '1970-12-31'), `release_time` ASC limit 1 ";


           //hash
            // if($hash == 1){
            //  $sql = "SELECT id, phone_number FROM `bananaapi-number` where `taked`=0 and `country_code` = ? and `application` = ? and `source` and type = 1 ORDER BY IFNULL(`release_time`, '1970-12-31'), `release_time` ASC  limit 1 ";
           
            // }
            // //Used
            // elseif ($hash == 2) {
            // 	 $sql = "SELECT id, phone_number FROM `bananaapi-number` where `taked`=0 and `country_code` = ? and `application` = ? and type = 3 ORDER BY IFNULL(`release_time`, '1970-12-31'), `release_time` ASC   limit 1 ";
            // }
            // //New
            // elseif ($hash == 3) {
            // 	 $sql = "SELECT id, phone_number FROM `bananaapi-number` where `taked`=0 and `country_code` = ? and `application` = ? and type = 2  ORDER BY IFNULL(`release_time`, '1970-12-31'), `release_time` ASC   limit 1 ";
            // }
           
           // $sql = "SELECT id, phone_number FROM `bananaapi-number` where `taked`=0 and `country_code` = ? and `application` = ? ";
           $sql .= "  FOR UPDATE";
           ...
           $row = $stmt->fetch(PDO::FETCH_ASSOC);
            ...
            if (strpos($randomSource, '_') !== false) {
                ...
                if (count($parts) >= 3) {
                    ...
                    if ($found) {
                        AddNumberToList($server,(int)$number,$list_id);
                    }
