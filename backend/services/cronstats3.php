       ...
       // var_dump($results);
        if (sizeof($results) > 0) {
            foreach ($results as $res) {               
                ...
                (SELECT COUNT(*) FROM `redis_numbers` WHERE taked = 1 and `country_code` = :country_code and `source` = :source and `redis_numbers`.`createdTime` = :currentday and is_finished = 1) AS has_sms,
        ...
        if (count($response) > 0) {
            ...
            foreach ($response as $key => $country) {
               ...
               // $jsonValue = $redis->get($redisKey);
                ...
                $no_sms = intval( $country["requested"]) - intval($country["has_sms"]);
